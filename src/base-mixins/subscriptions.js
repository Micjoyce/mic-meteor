 ...
 *     (under the `subscriptions.cache` property of the Meteor instance) of ddp
...
/*
 *   Private methods: they are invoked with the Meteor instance as context, but
 *   they are not exported so they don't clutter the Meteor class prototype.
 */
...
export function subscribe(name, ...params) {
  ...
  let sub = this.subscriptions.cache.get(fingerprint);
  const self = this;
  if (!sub) {
    ...
    sub = assign(
      new EventEmitter(), {
        ...
        stillInQueue,
        stop: function() {
          self.ddp.unsub(this.id);
          self.subscriptions.cache.del(this.id);
        }
      }
    );
